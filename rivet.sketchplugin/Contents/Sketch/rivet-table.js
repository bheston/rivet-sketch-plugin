...
var Table = function Table() {
  var BORDER_WIDTH = 1; // Parts

  var headers, columns, widths, metas, props; // Symbols & Groups

  var table, tableMask, tableGroup;
  var tableToReplace = null; // TODO

  var layerForPositioning = null; // Counts
  ...
  var _generateTable = function _generateTable() {
    var initialData = null;
    var sel = _common_context__WEBPACK_IMPORTED_MODULE_0__["context"].current().selection;
    var selCount = sel.count();

    if (selCount == 1) {
      if (_common_sketch__WEBPACK_IMPORTED_MODULE_5__["sketch"].is(sel[0], MSLayerGroup)) {
        initialData = _common_metadata__WEBPACK_IMPORTED_MODULE_2__["metadata"].getConfigs(sel[0]);
        _common_logger__WEBPACK_IMPORTED_MODULE_1__["logger"].debug('loading config', initialData);
      }

      if (tableToReplace == null) {
        // Use for positioning
        if (!_common_sketch__WEBPACK_IMPORTED_MODULE_5__["sketch"].is(sel[0], MSArtboardGroup)) {
  ...
  var _tablePanel = function _tablePanel(callback, data) {
    return _common_panel__WEBPACK_IMPORTED_MODULE_3__["panel"].createPanel({
      url: _common_context__WEBPACK_IMPORTED_MODULE_0__["context"].getPluginRoot() + "/Contents/Resources/panel/table.html",
      width: 1180,
      height: 700,
      data: data,
  ...
  var _parseDataFromPanel = function _parseDataFromPanel(data) {
    var _parseDataFromString = function _parseDataFromString(str, sep) {
  ...
  var _getTableSize = function _getTableSize() {
    rowCount = columns[0].length;
    _common_logger__WEBPACK_IMPORTED_MODULE_1__["logger"].debug('rowCount - ' + rowCount);
  };

  var _createGroup = function _createGroup() {
    tableGroup = _common_sketch__WEBPACK_IMPORTED_MODULE_5__["sketch"].addGroup();
    tableGroup.setName('Table');
  };

  var _storeConfig = function _storeConfig() {
    _common_metadata__WEBPACK_IMPORTED_MODULE_2__["metadata"].setConfigs(props, tableGroup);
  };

  var _addPagination = function _addPagination() {
    if (props.hasPagination == 'off') {
      return;
    }

    var symbol = _common_search__WEBPACK_IMPORTED_MODULE_4__["search"].findSymbolsByName('Rivet/Table/Paging')[0];
    var instance = symbol.newSymbolInstance();
    instance.setName('Paging');
    tableGroup.addLayers([instance]);
    var rect = _common_sketch__WEBPACK_IMPORTED_MODULE_5__["sketch"].getRect(instance);
    rect.setY(395);
    rect.setX(0); //var action = MSMoveToBackAction.alloc().init();
    //[action moveToBack:instance];
  };

  var _addTableBase = function _addTableBase() {
    tableMask = _common_sketch__WEBPACK_IMPORTED_MODULE_5__["sketch"].addShape();
    tableMask.setName('Mask');
    _common_sketch__WEBPACK_IMPORTED_MODULE_5__["sketch"].setAsMask(tableMask); //if (props.hasShadow == 'on') {
    //sketch.setRadius(tableMask, 0);

    var maskStyle = _common_search__WEBPACK_IMPORTED_MODULE_4__["search"].findLayerStyleByName('Rivet Table Mask');
    tableMask.setSharedStyle(maskStyle); //}

    var maskRect = _common_sketch__WEBPACK_IMPORTED_MODULE_5__["sketch"].getRect(tableMask);
    var symbol = _common_search__WEBPACK_IMPORTED_MODULE_4__["search"].findSymbolsByName('Rivet/Table/Table')[0];
    table = symbol.newSymbolInstance();
    table.setName('Table');
    var tableRect = _common_sketch__WEBPACK_IMPORTED_MODULE_5__["sketch"].getRect(table);
  ...
  var _makeCols = function _makeCols() {
    var x = BORDER_WIDTH;
    var symbol = _common_search__WEBPACK_IMPORTED_MODULE_4__["search"].findSymbolsByName('Rivet/Table/Column-Dimension')[0];
    var metricSymbol = _common_search__WEBPACK_IMPORTED_MODULE_4__["search"].findSymbolsByName('Rivet/Table/Column-Metric-Number')[0];
    var iconSymbol = _common_search__WEBPACK_IMPORTED_MODULE_4__["search"].findSymbolsByName('Rivet/Table/Column-Icon')[0];

    if (props.hasCheckboxes == 'on') {
      var selectSymbol = _common_search__WEBPACK_IMPORTED_MODULE_4__["search"].findSymbolsByName('Rivet/Table/Column-Select')[0];
    ...
    for (var i = 0; i < headers.length; i++) {
      ...
      tableGroup.addLayers([instance]);
      _common_sketch__WEBPACK_IMPORTED_MODULE_5__["sketch"].moveLayerAbove(tableGroup, table, instance);
      var overrideValues = {
        'Title': header
      };

      if (i == headers.length - 1) {
        overrideValues['Last Column'] = {
          'symbolID': _common_sketch__WEBPACK_IMPORTED_MODULE_5__["sketch"].OVERRIDE_NULL_SYMBOL
        };
      }
      ...
      for (var j = 0; j < 10; j++) {
        ...
        if (props.hasDefaultValues == 'off' && value == '') {
          value = ' ';
        }

        if (isIcon && value.toLowerCase() != 'x') {
          _common_logger__WEBPACK_IMPORTED_MODULE_1__["logger"].debug('Setting icon to null for row ' + (j + 1));
          value = {
            'symbolID': _common_sketch__WEBPACK_IMPORTED_MODULE_5__["sketch"].OVERRIDE_NULL_SYMBOL
          };
        }
      ...
      _common_sketch__WEBPACK_IMPORTED_MODULE_5__["sketch"].updateOverrides(instance, overrideValues);
      ...
      var columnRect = _common_sketch__WEBPACK_IMPORTED_MODULE_5__["sketch"].getRect(instance);
      columnRect.setX(x);
  ...
  var _addGroupsToTable = function _addGroupsToTable() {
    tableGroup.setConstrainProportions(false);
    tableGroup.select_byExpandingSelection(true, true);
  };

  var _positionOrReplaceTable = function _positionOrReplaceTable() {
    if (tableToReplace != null) {
      ...
      _common_sketch__WEBPACK_IMPORTED_MODULE_5__["sketch"].moveLayerAbove(pageOrArtboard, layerForPositioning, tableGroup);
      var layerRect = _common_sketch__WEBPACK_IMPORTED_MODULE_5__["sketch"].getRect(layerForPositioning);
      var tableGroupRect = _common_sketch__WEBPACK_IMPORTED_MODULE_5__["sketch"].getRect(tableGroup);
      tableGroupRect.setX(layerRect.x);
      tableGroupRect.setY(layerRect.y);
    }

    tableGroup.select_byExpandingSelection(true, false); //tableGroup.select_byExpandingSelection(true, true);
  };
...
//# sourceMappingURL=rivet-table.js.map